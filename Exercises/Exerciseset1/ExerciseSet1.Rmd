...
2. For Apple: the cumulative return in the period is `r round(100*sum(StockReturns[,2]),1)`%; the average daily return is `r round(100*mean(StockReturns[,2]),3)`%; the standard deviation of the daily returns is `r round(100*sd(StockReturns[,2]),3)`%.
...
2. Script dataSet1AB.R pulls returns of ```r source("dataSet1AB.R"); mytickers;```. The code below plots the returns of an equal weighted portfolio.
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio = apply(StockReturns,1,weighted.mean)
names(portfolio) <- rownames(StockReturns)
pnl_plot(portfolio)
```
...
cbinvestments<-read.csv("investments.csv",header=TRUE,nrows=1000)
cbinvestments[,"raised_amount_usd"][is.na(cbinvestments[,"raised_amount_usd"])]<-0
mystat <- c(max(cbinvestments[,"raised_amount_usd"]),mean(cbinvestments[,"raised_amount_usd"]),sd(cbinvestments[,"raised_amount_usd"]))
names(mystat)<-c("MaxInvestment","AvgInvestment","StdDev")
print("Below are basic statistics of a selection of 1,000 investments from CrunchBase:")
print.table(format(mystat,nsmall=0,scientific = FALSE,big.mark = ","),digits=8)
print("Some info about the maximum investment:")
grid.table(cbinvestments[which(cbinvestments[,"raised_amount_usd"] == max(cbinvestments[,"raised_amount_usd"])),c("company_name", "company_category_list","investor_name","funding_round_type")])
```
